...
export async function sendEvm(
    ...
    const signer = await srcEidHre.ethers.getNamedSigner('deployer') as any
    ...
    // 5.1️⃣ preflight: ensure the sender has enough balance of the underlying token
    const balance: BigNumber = await erc20.balanceOf(signer.address)
    if (amountUnits.gt(balance)) {
        DebugLogger.printErrorAndFixSuggestion(
            KnownErrors.ERROR_SENDING_TRANSACTION,
            `Insufficient token balance on ${endpointIdToNetwork(srcEid)} for ${signer.address}. ` +
                `Balance=${formatUnits(balance, decimals)}, attempted=${formatUnits(amountUnits, decimals)}. ` +
                `Reduce --amount or mint/transfer more tokens to the sender.`
        )
        throw new Error('Insufficient token balance for OFT send')
    }
